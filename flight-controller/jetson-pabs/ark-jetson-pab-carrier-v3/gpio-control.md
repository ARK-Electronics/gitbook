...
Unlike the original PAB Carrier and Just a Jetson, the V3 does not route the Jetson's I2S0 pins to a dedicated GPIO connector. Jetson GPIO on the V3 is available on the **Pixhawk Payload Bus** and avionics connectors — see the [Pinout](pinout.md) for which pins carry GPIO-capable signals.
