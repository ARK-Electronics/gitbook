...
| [UAVCAN\_SUB\_IMU](https://docs.px4.io/main/en/advanced_config/parameter_reference.html#UAVCAN_SUB_IMU) | Set to `1` to subscribe to DroneCAN `RawIMU` messages. Requires `CANNODE_PUB_IMU` to also be set on the GPS |
...
| `CANNODE_PUB_IMU` | Set to `1` to publish `RawIMU` messages on the CAN bus. Requires `UAVCAN_SUB_IMU` to also be set on the autopilot |
