...
|-----------|-------------|
| [UAVCAN\_SUB\_IMU](https://docs.px4.io/main/en/advanced_config/parameter_reference.html#UAVCAN_SUB_IMU) | Set to `1` to subscribe to DroneCAN [`RawIMU`](../../knowledge-base/dronecan-messages.md#rawimu) messages. Requires `CANNODE_PUB_IMU` to also be set on the ARK Flow MR |
...
| `CANNODE_PUB_IMU` | Set to `1` to publish [`RawIMU`](../../knowledge-base/dronecan-messages.md#rawimu) messages on the CAN bus. Requires `UAVCAN_SUB_IMU` to also be set on the flight controller |
